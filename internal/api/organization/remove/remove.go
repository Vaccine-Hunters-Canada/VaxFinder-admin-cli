...
func (HTTPOperation) GetDetails() (string, string, string) {
	return "remove", "removed", "organization " + strconv.Itoa(id)
}
...
func (HTTPOperation) GetAsCurlCommand(withKey bool) (*http2curl.CurlCommand, error) {
	...
	req, rErr := api.NewDeleteOrganizationByIdApiV1OrganizationsOrganizationIdDeleteRequest(utils.GetBaseURL(), id)
	if rErr != nil {
		return nil, rErr
	}
