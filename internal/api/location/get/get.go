...
func (HTTPOperation) GetDetails() (string, string, string) {
	return "get", "got", "location " + strconv.Itoa(id)
}
...
func (HTTPOperation) GetResponseAsArray() ([][]string, error) {
	...
	if res.JSON200 != nil {
		return [][]string{
			location.ConvertExpandedLocationJSONToTableRow(res.JSON200),
		}, nil
	}
...
func (HTTPOperation) GetAsCurlCommand(withKey bool) (*http2curl.CurlCommand, error) {
	...
	req, rErr := api.NewRetrieveLocationByIdApiV1LocationsLocationIdGetRequest(utils.GetBaseURL(), id)
	if rErr != nil {
		return nil, rErr
	}
