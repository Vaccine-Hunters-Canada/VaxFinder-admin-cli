...
// CmdRunE is what's executed when running `vf-admin organization remove <id>`
func CmdRunE(cmd *cobra.Command, args []string) error {
	...
	// Create spinner
	spinner, sErr := utils.GetDefaultSpinnerForHTTPOp("remove", "removed", fmt.Sprint("organization ", id))
	if sErr != nil {
	...
	res, rErr := client.DeleteOrganizationByIdApiV1OrganizationsOrganizationIdDeleteWithResponse(context.Background(), id)
