...
// CmdRunE is what's executed when running `vf-admin requirement remove <id>`
func CmdRunE(cmd *cobra.Command, args []string) error {
	...
	// Create spinner
	spinner, sErr := utils.GetDefaultSpinnerForHTTPOp("remove", "removed", fmt.Sprint("requirement ", id))
	if sErr != nil {
	...
	res, rErr := client.DeleteRequirementByIdApiV1RequirementsRequirementIdDeleteWithResponse(context.Background(), id)
