...
// CmdRunE is what's executed when running `vf-admin address update`
func CmdRunE(cmd *cobra.Command, args []string) error {
	...
	body := api.UpdateAddressApiV1AddressesAddressIdPutJSONRequestBody{Province: province, Postcode: postcode, Latitude: latitude, Longitude: longitude, Line1: line1, Line2: line2, City: city}

	// Create spinner
	spinner, sErr := utils.GetDefaultSpinnerForHTTPOp("update", "updated", "address")
	if sErr != nil {
	...
	res, rErr := client.UpdateAddressApiV1AddressesAddressIdPutWithResponse(context.Background(), id, body)
