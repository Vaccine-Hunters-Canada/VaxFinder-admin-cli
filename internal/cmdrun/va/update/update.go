...
// CmdRunE is what's executed when running `vf-admin va update <id>`
func CmdRunE(cmd *cobra.Command, args []string) error {
	...
	body := api.UpdateVaccineAvailabilityApiV1VaccineAvailabilityVaccineAvailabilityIdPutJSONRequestBody{Date: date, Location: location, InputType: inputType, NumberAvailable: numberAvailable, NumberTotal: numberTotal, Vaccine: vaccine, Tags: tags}
