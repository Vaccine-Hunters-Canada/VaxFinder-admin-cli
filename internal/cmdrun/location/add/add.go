...
// CmdRunE is what's executed when running `vf-admin location add`
func CmdRunE(cmd *cobra.Command, args []string) error {
	...
	body := api.CreateLocationApiV1LocationsPostJSONRequestBody{Active: active, Name: name, Postcode: postcode, Phone: phone, Notes: notes, Url: url, Tags: tags, Organization: org, Address: address}

	// Create spinner
	spinner, sErr := utils.GetDefaultSpinnerForHTTPOp("add", "added", "location")
	if sErr != nil {
